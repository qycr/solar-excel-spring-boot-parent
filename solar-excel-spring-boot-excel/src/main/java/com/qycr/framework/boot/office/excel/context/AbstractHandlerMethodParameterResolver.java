...
public abstract class AbstractHandlerMethodParameterResolver extends WebApplicationObjectSupport implements InitializingBean, EmbeddedValueResolverAware, DisposableBean {
    ...
    protected List<ResolverFilter> getResolverFilters() {
        if (resolverFilters == null) {
            ...
            resolverFilters = applicationContext.getBeanProvider(ResolverFilter.class).stream().
                    filter(resolverFilter -> resolverFilter.filterType() == FilterType.REQUEST ||
